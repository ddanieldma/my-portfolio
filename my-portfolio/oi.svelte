<script>
    ...
    // Create a filtered version of commits based on commitMaxTime
    $: filteredCommits = commits.filter(commit => commit.datetime <= commitMaxTime)
    
    // Get min and max dates for filtered commits (with fallbacks to original dates)
    $: filteredMinDate = filteredCommits.length > 0 ? d3.min(filteredCommits, d => d.date) : minDate
    $: filteredMaxDate = filteredCommits.length > 0 ? d3.max(filteredCommits, d => d.date) : maxDate
    ...
    $: statsData = [
        ...
        d3.groups(selectedLines, d => d.file).length,
    ...
    $: rScale = d3.scaleSqrt()
        .domain(d3.extent(filteredCommits.length > 0 ? filteredCommits.map(d => d.totalLines) : [0, 1]))
        .range([2, 30])
